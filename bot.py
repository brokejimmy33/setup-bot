...
@tree.command(name="setup", description="Create server categories, channels, and roles")
...
async def setup(interaction: Interaction):
    ...
    # Existing roles
    roles = {
        "🥔 The Ultimate Admin": discord.Permissions(administrator=True),
        "🥄 Mashed Mod": discord.Permissions(kick_members=True, manage_messages=True),
        "👥 Potato People": discord.Permissions(send_messages=True, read_messages=True),
        "🥔 Guest Tater": discord.Permissions(read_messages=True, send_messages=False),
        # New fun roles
        "🎉 Party People": discord.Permissions(send_messages=True, read_messages=True),
        "🧠 Big Brain": discord.Permissions(send_messages=True, read_messages=True),
        "🔧 Tech Wizard": discord.Permissions(send_messages=True, read_messages=True),
        "🎮 Gamer": discord.Permissions(send_messages=True, read_messages=True),
        "📚 Learner": discord.Permissions(send_messages=True, read_messages=True),
        "👑 Server Founder": discord.Permissions(administrator=True)
    }
    ...
    admin_roles = [created_roles["🥔 The Ultimate Admin"], created_roles["🥄 Mashed Mod"], created_roles["👑 Server Founder"]]
    ...
    for r in admin_roles:
        overwrites_admin_only[r] = discord.PermissionOverwrite(view_channel=True, send_messages=True)
    ...
    logs_cat = await create_category("📢 Server Info / Logs", overwrites_admin_only)
    ...
    # 5 - Study / Work / Tech
    study_cat = await create_category("📚 Study / Work / Tech", overwrites_public)
    ...
    for r in admin_roles:
        overwrites_private[r] = discord.PermissionOverwrite(view_channel=True, connect=True, speak=True)
    private_vc = discord.utils.get(guild.voice_channels, name="Private Room")
    if not private_vc:
        await guild.create_voice_channel("Private Room", category=voice_cat, overwrites=overwrites_private)
    ...
    await interaction.followup.send("✅ Server setup complete with new categories, channels, and roles added!")


@tree.command(name="setlogchannel", description="Set the channel where logs will be sent")
...
@tree.command(name="autorole", description="Choose a role to auto-assign to new members")
...
@tree.command(name="config", description="Show all current configuration settings")
@app_commands.checks.has_permissions(administrator=True)
async def config_show(interaction: Interaction):
    ...
    embed.add_field(name="👤 Auto Role", value=auto_role.mention if auto_role else "❌ Not set", inline=False)
...
async def show_commands(interaction: Interaction):
    ...
    embed.add_field(name="/setup", value="Create server categories and roles", inline=False)
    embed.add_field(name="/setlogchannel", value="Set the log channel for moderation actions", inline=False)
    embed.add_field(name="/autorole", value="Set an auto-role for new members", inline=False)
    embed.add_field(name="/config", value="Show all current config values", inline=False)
    embed.add_field(name="/mute", value="Temporarily mute a member", inline=False)
    ...
    embed.add_field(name="/bots", value="Show recommended bots for your server", inline=False)
    embed.add_field(name="/resync", value="Force re-sync of commands (Admin only)", inline=False)

    embed.set_footer(text="Some commands require special permissions (Admin, Mod, etc.)")
    await interaction.response.send_message(embed=embed)

@tree.command(name="bots", description="Show recommended bots")
async def bots(interaction: Interaction):
    embed = discord.Embed(
        title="🥔 Recommended Bots for The Ultimate Potato",
        description="Useful bots to help your server grow!",
        color=discord.Color.gold()
    )
    embed.add_field(name="🛡️ Moderation", value="[Carl-bot](https://carl.gg/invite)\n[Dyno](https://dyno.gg/invite)", inline=False)
    embed.add_field(name="📊 Stats & Tickets", value="[Statbot](https://statbot.net/invite)\n[Ticket Tool](https://tickettool.xyz/direct-invite)", inline=False)
    embed.add_field(name="🎉 Fun & XP", value="[Arcane](https://arcane.bot/invite)\n[Dank Memer](https://dankmemer.lol/invite)", inline=False)
...
async def mute(interaction: Interaction, member: discord.Member, duration: int):
    guild = interaction.guild
    ...
    await log_action(guild, f"⛔ {member} was muted for {duration} min by {interaction.user}.")
...
async def unmute(interaction: Interaction, member: discord.Member):
    ...
    if mute_role in member.roles:
        ...
        await log_action(interaction.guild, f"🔈 {member} was manually unmuted by {interaction.user}.")
    else:
...
async def ban(interaction: Interaction, member: discord.Member, reason: str = "No reason provided"):
    ...
    await log_action(interaction.guild, f"🔨 {member} was banned by {interaction.user}: {reason}")
...
async def kick(interaction: Interaction, member: discord.Member, reason: str = "No reason provided"):
    ...
    await log_action(interaction.guild, f"👢 {member} was kicked by {interaction.user}: {reason}")
